...
static const char *TAG = "BLUFI_SIM";
...
static void example_wifi_connect(void)
{
    ...
    //if (gl_sta_is_connecting == true)
    //{
    //    memcpy(ui_state.wifi_state.CONNECTED_SSID, gl_sta_conn_info.sta_ssid, gl_sta_conn_info.sta_ssid_len);
    //    BLUFI_INFO("Connected SOFTAP SSID %s \n", ui_state.wifi_state.CONNECTED_SSID);
    //}
...
static void get_current_rssi()
{
    wifi_ap_record_t ap_info;
    esp_err_t err = esp_wifi_sta_get_ap_info(&ap_info);
    if (err == ESP_OK)
    {
        ui_state.wifi_state.test_RSSI = ap_info.rssi;
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    case WIFI_EVENT_STA_CONNECTED:
        ...
        memcpy(ui_state.wifi_state.CONNECTED_SSID, event->ssid, event->ssid_len);
        BLUFI_INFO("Connected SOFTAP SSID %s \n", ui_state.wifi_state.CONNECTED_SSID);
        // BLUFI_INFO("Connected SOFTAP PASSWORD %s \n", sta_config.sta.password);

        cie_set_static_ip(); // 设置静态ip
        get_current_rssi();
...
// 模拟 BLUFI 事件
void cie_simulate_blufi_event(esp_blufi_cb_event_t event, uint8_t *data, size_t data_len)
{
    esp_blufi_cb_param_t param;
    // uint8_t *ssid_buffer = malloc(strlen((const char*)data) + 1);
    // strcpy((char*)ssid_buffer, (const char*)data);

    switch (event)
    {
    case ESP_BLUFI_EVENT_RECV_STA_SSID:
        param.sta_ssid.ssid = data;
        param.sta_ssid.ssid_len = data_len;
        break;
    case ESP_BLUFI_EVENT_RECV_STA_PASSWD:
        param.sta_passwd.passwd = data;
        param.sta_passwd.passwd_len = data_len;
        break;
    default:
        ESP_LOGE(TAG, "Unsupported event: %d", event);
        return;
    }

    // 直接调用回调函数  example_event_callback
    // example_event_callback(ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP, &param);
    example_event_callback(event, &param);
}

// 示例：主动设置 SSID 和密码
void cie_change_ssid_password(const char *ssid, const char *password)
{
    esp_blufi_cb_param_t param;
    // 触发 SSID 事件
    cie_simulate_blufi_event(ESP_BLUFI_EVENT_RECV_STA_SSID, (uint8_t *)ssid, strlen(ssid));

    // 触发密码事件
    cie_simulate_blufi_event(ESP_BLUFI_EVENT_RECV_STA_PASSWD, (uint8_t *)password, strlen(password));

    example_event_callback(ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP, &param);
}

void cie_wifi_manage(void)
{
    esp_err_t ret;
    ...
    // vTaskDelay(3000);
    // BLUFI_INFO("cie_change_ssid_password \n");
    // cie_change_ssid_password("CIE", "15817481011");
}
