...
extern char cie_ssid[33];
extern char cie_password[65];
...
static const char *TAG = "cie_wifi";
                                               ...
                                               int32_t event_id, void *event_data)
{
    ...
    if (s_retry_num > CIE_WIFI_CONN_MAX_RETRY)
    {
        ESP_LOGI(TAG, "WiFi Connect failed %d times, stop reconnect.", s_retry_num);
        //#if CIE_PROJECT
        
        #if CIE_CLEAR_SSID_PW
        cie_clear_SSID_PW_from_flash();
        #endif
    ...
    ESP_LOGI(TAG, "Wi-Fi disconnected %d, trying to reconnect...", disconn->reason); // 断开了连接
    ui_state.wifi_state.connect_flag = false;

    esp_err_t err = esp_wifi_connect(); // 重先连接

    if (err == ESP_OK)
    {
        ;
        //ui_state.wifi_state.connect_flag = true;
    }
...
{
    ESP_LOGI(TAG, "cie  example_wifi_start...");
...
esp_err_t example_wifi_sta_do_connect(wifi_config_t wifi_config, bool wait)
{
    ...
    if (wait)
    {
        ESP_LOGI(TAG, "Waiting for IP(s)"); // 之后
    ...
    //ESP_LOGI(TAG, "CIE example_wifi_sta_do_connect return..."); // 之前
    return ESP_OK;
}
...
// CIE_PROJECT  写入ssid password
esp_err_t cie_wifi_connect(void)
{
    ESP_LOGI(TAG, "CIE Start example_connect.");
    example_wifi_start();
    wifi_config_t wifi_config = {
        .sta = {
#if !CONFIG_EXAMPLE_WIFI_SSID_PWD_FROM_STDIN  
           // .ssid = CIE_WIFI_SSID,               //CIE_WIFI_SSID,         //"SwiftChen的iPhone",  //CONFIG_EXAMPLE_WIFI_SSID,
           // .password = CIE_WIFI_PASSWORD,       //CIE_WIFI_PASSWORD, //"chenchen88176",      //CONFIG_EXAMPLE_WIFI_PASSWORD,
    ...
    memcpy(wifi_config.sta.ssid, ui_state.wifi_state.SSID, sizeof(wifi_config.sta.ssid));
    memcpy(wifi_config.sta.password, ui_state.wifi_state.PASSWORD, sizeof(wifi_config.sta.ssid));
